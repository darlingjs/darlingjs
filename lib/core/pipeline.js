...
var Pipeline = function(pipeline, system) {
   ...
   * @returns {Function}
   */
  function wrapHandlerForEachEntity(async, handler) {
    if (async) {
  ...
  if (system) {
    ...
    this.async = pipeline.async || system.async;

    if (system.beforeUpdate) {
      this.beforeUpdateList = pipeline.beforeUpdateList.clone();
      this.beforeUpdateList.add(wrapHandlerForEachEntity(this.async, function(entity, interval) {
    ...
    if (system.updateOne) {
      ...
      this.updateOneList.add(wrapHandlerForEachEntity(this.async, function (entity, interval) {
    ...
    if (system.afterUpdate) {
      ...
      this.afterUpdateList.add(wrapHandlerForEachEntity(this.async, function(entity, interval) {
 ...
 * @param state
 */
Pipeline.prototype.step = function(interval, state) {
