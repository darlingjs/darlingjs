...
(function() {
     ...
     * @type {Function}
     */
    Module.prototype.s = Module.prototype.system = function() {
        return {};
    };

    var World = function(){};

    World.prototype._components = {};
    World.prototype._modules = {};

    World.prototype.has = function(name) {
        return isDefined(this._components[name]) || isDefined(this._modules[name]);
    }
        ...
        for (var index = 0, count = components.length; index < count; index++) {
            if (isString(components[index])) {
                ...
                if (isUndefined(component)) {
                    throw Error('World ' + this.name + ' doesn\'t has component ' + componentName + '. Only ' + this._components);
                }
    ...
    GameEngine.w = GameEngine.world = function(name, requires) {
        ...
        if (isArray(requires)) {
            for (var index = 0, count = requires.length; index < count; index++) {
                ...
                worldInstance._modules[moduleName] = module;

                for (var key in module._components) {
                    var component = module._components[key];
                    if (component === null) {
                        throw Error('Module: ' + name + ' has null component');
                    }

                    worldInstance._components[component.name] = component;
                }
            }
        }
